...
| **Status** | 360 unit tests, 102 consistency checks, 2 evidence bundles replaying clean |
...
### On repositories nobody wrote it for

Fixture tests prove the three layouts are handled, which isolates the variable
and does not answer whether this survives contact with real code. So the whole
workflow is run against pinned public repositories, described only by a
`.placebo.toml`, with no Placebo source edited:

```bash
python scripts/run_external_repos.py
```

| repository | pinned at | doctor | oracles sourced | mutation score |
|---|---|---|---:|---:|
| `inflection` 0.5.1 | `b00d4d34` | supported | 39 | 74.2% (8 undetected) |
| `pathspec` v1.1.1 | `ecf71a99` | supported | 0 | 64.3% (5 undetected) |
| `toml-sort` v0.9.0 | `c1655661` | supported | 0 | refused |

**Two of three completed, and the third is reported rather than dropped.**
`toml-sort`'s own suite needs `tomlkit` installed, which Placebo does not do.
Its baseline is therefore red, and a red baseline makes every downstream
verdict meaningless, so the census refuses instead of producing numbers. That
is the tool behaving correctly on a repository it cannot yet handle.

Building this found a real defect. `test_roots` from the contract was never
passed to the runner, which used its own `tests` default, so any repository
keeping its suite elsewhere could not be censused at all. `inflection` 0.5.1
keeps 467 tests in a single file at the repository root. The fixture tests
could not have caught that: they all happened to use `tests/`.
...
python -m pytest tests             # 360 passed
